...
Every growth figure in this paper is a GDP figure, and GDP measures production inside a boundary set by the System of National Accounts. That boundary is a convention, and the clearest demonstration is what it already imputes: if a household owns the dwelling it occupies, the accounts estimate the rent it would otherwise have paid and add that imputed sum to output, on the reasoning that a real service is being consumed. The identical reasoning is declined for a meal cooked, a child minded, or an elderly parent nursed within the same household. Own-account household services sit outside the boundary and are recorded not as poorly estimated but as zero. The same framework imputes a rent for the building and nothing for the person working inside it.

The feasibility defence is weak in the Indian case, because the hours are already counted. The National Statistical Office conducted Time Use Surveys over January–December 2019 and January–December 2024, the second covering roughly 4.5 lakh people in 1.3 lakh households. Among those aged 15–59 who perform it at all, women spent 289 minutes a day on unpaid domestic services in 2024 against 88 minutes for men — about 1,758 hours a year for a woman, which appears in no employment statistic and no output series. On caregiving, 41 per cent of women in that age band participated against 21.4 per cent of men, at 140 minutes against 74.

Two rounds permit a measured statement about change rather than an asserted one, and the result is not the simple story in either direction. Men's participation nearly doubled, from 26.1 per cent in 2019 to 45.8 per cent in 2024. Yet minutes among participants fell for both sexes — men 97 to 88, women 299 to 289 — so the participant gap moved from 202 minutes to 201. **One minute in five years.** Over the same period women's participation in employment rose from 21.8 to 25.0 per cent: more women in paid work, with the unpaid work not meaningfully redistributed.

Published Indian valuations of unpaid household work range from 7.5 per cent of GDP (SBI Ecowrap 2023, minimum-wage valuation) through a government working range of 15–17 per cent, to 24.6 per cent on generalised opportunity cost and 32.4 per cent on replacement cost for 2019–20, and 26–36 per cent in Economic and Political Weekly (2024) for 2022–23. The spread is a factor of nearly five and is driven almost entirely by valuation method rather than by measurement error. We do not adjudicate between them and publish no preferred figure. The narrower claim we do make is that every one of these estimates is large, and that the reason none can be declared correct is that no statistical agency has been asked to settle it.

This bears directly on a figure used elsewhere in this paper. Female labour force participation of roughly 21 per cent is customarily discussed as an absence of work, while the Time Use Survey records 83.9 per cent of women aged 15–59 performing unpaid domestic work on the reference day. Both are true. Together they indicate that the participation figure measures the absence of work *inside the boundary*, and how much of India's female participation gap is behavioural rather than definitional is a real question this paper has not answered.

We therefore adjust nothing. Every growth series here remains the official one on the official boundary. What we assert is that the boundary is a choice, that India already collects the hours required to value what lies outside it, that the excluded quantity is plausibly between a thirteenth and a third of GDP, and that a framework imputing rent for a dwelling while recording zero for the care performed within it embeds a judgement about whose work constitutes production. A paper arguing that measurement is political cannot quietly accept the most consequential measurement convention of all. A household-production satellite account, which several statistical systems maintain and India's does not, would replace this section with a number.
